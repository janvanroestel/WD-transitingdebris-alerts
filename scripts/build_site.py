...
<head>
    ...
    <style>
        ...
        @media (max-width: 720px) {{
            ...
            .recent-alerts th, .recent-alerts td {{ padding: 4px 8px 4px 0; font-size: 13px; }}
            .plot-container {{ padding: 4px; margin-bottom: 18px; }}
...
<body>
    ...
    <div class="controls">
        ...
        <button class="theme-btn" id="theme-btn" aria-label="Toggle dark mode">Dark</button>
    </div>
...
function getLayout(objData, mode) {{
    ...
    const gridColor = isDark ? "#2a2f35" : "#e6e6e6";
...
document.querySelectorAll(".alert-link").forEach(link => {{
    link.addEventListener("click", function(e) {{
        ...
        if (groupChanged) requestAnimationFrame(() => requestAnimationFrame(doScroll));
...
// Re-render on mobile/desktop breakpoint changes so plot sizing updates
let lastMobile = window.matchMedia("(max-width: 720px)").matches;
